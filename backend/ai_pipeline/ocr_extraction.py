...
# gpt-4o, gpt-4o-mini, gpt-4 (cross-check with the provider)
input_model = "gpt-4o"
...
def convert_pdf_to_images(pdf_path: str) -> list[str]:
    ...
    Temporary image files are created in the 'temp' directory.
    """
    try:
        pages = convert_from_path(pdf_path, poppler_path=poppler_path)
        image_paths = []
        for i, page in enumerate(pages):
            image_file = os.path.join(TEMP_DIR, f"{os.path.splitext(os.path.basename(pdf_path))[0]}_page_{i}.jpg")
...
def ocr_extraction(file_list: list[str], prompt="Extract text from image") -> str:
    ...
    for file_path in file_list:
        ...
        for image in image_files:
            ...
            if file_path.lower().endswith(".pdf") and image.startswith(TEMP_DIR):
                try:
                    os.remove(image)
                except Exception as e:
...
if __name__ == "__main__":
    files = ["tests/example_qn_paper_mini.pdf"]
